...
//https://byteshiva.medium.com/how-to-read-and-extract-information-from-a-pcap-file-in-go-287c0bd66561
...
func main() {
	...
	for packet := range packetSource.Packets() {
		...
		if ipLayer != nil {
			...
			if len(ipPacket.Payload) >= 30 {
				fmt.Println("------------------------------------------------------------------------------------------------")
				fmt.Println("IP source address:", ipPacket.SrcIP)
				fmt.Println("IP destination address:", ipPacket.DstIP)

				fmt.Println("Len: ", len(ipPacket.Payload))
				fmt.Println("IP Payload:", string(ipPacket.Payload))
